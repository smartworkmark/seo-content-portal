...
- Action dot color coding (movement-based via `actionDotCounts()`): red = pending pause; amber = pending approval; blue = an auto change that actually moves the live budget (`direction !== 'flat'`); flat rows show nothing. **The Actions column renders dots ONLY** — when no campaign needs an action the cell is empty (no "On track" pill, no `—` dash). The account's state is already conveyed by the Status column, so duplicating it here was redundant. `hasAppliedChange()` is still used for row dimming (see below).
