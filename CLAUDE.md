...
- Action dot color coding (movement-based via `actionDotCounts()`): red = pending pause; amber = pending approval; blue = an auto change that actually moves the live budget (`direction !== 'flat'`); flat rows show nothing. On-track accounts show the green "On track" pill ONLY when no campaign moved the budget (`hasAppliedChange()` false) — once DOW shaping is live an on-track account can still have applied moves and shows dots.
...
- **Applied-budget rendering (rows with `finalDailyBudget`)** — the "Proposed /day" column becomes "Applied /day" (= `final`, or the if-approved target for approval/pause rows) and "Change" = `current → final` %; the "Recommendation" column becomes a derived **Status** pill from `appliedStatusLabel()` (`Auto-applied` / `Pending approval` / `Pause (pending)` / `No change`), replacing the raw recommendation badge. A muted "why" subtext is assembled from: `'day-of-week shaping'` (when `shouldShowDowBanner` and the row moved), `'auto-applied X% cut'` (when `autoDecreasePromoted`), and the skip-reason label **only when `direction === 'flat'`**. Flat rows show `—` for Applied/Change.
...
- Source tab name in the spreadsheet is **`KW Buildout Proposals`** (22 columns, one row per proposed keyword). Like the other newer sheets, the fetch is wrapped in `.catch(() => [])` for graceful empty fallback. Parser matches columns by header NAME (case-insensitive), so column order is irrelevant.
- Columns: account_id, account_name, campaign_id, campaign_name, ad_group_id, ad_group_name, proposed_keyword, proposed_match_type, match_type_basis, source_search_term, search_term_status, served_by_keyword, conversions, cost, cpa, confidence, flags, needs_new_ad_group, status, logged_at, batch_id, approval, proposal_id
...
- `parseKwBuildout()` in `google-sheets.ts` groups rows into one `KwBuildoutRecord` per batch with a `keywords[]` array. Account-level fields (accountId, accountName, loggedAt) come from the first row, filled from later rows if empty.
...
- **No error mode, no URL validation.** Practice filter keys on `accountName` (there is no companyId/HubSpot link; `account_id` is the Google Ads ID). Uses the **short-range pills (1d/3d/7d)** — added to `SHORT_RANGE_TABS`.
...
- On submit, the panel calls `submitKwBuildoutFeedback(record, approvedKeys)` (hook) which POSTs `{ batch_id, account_id, account_name, logged_at, approval_status: 'Approved', approved: [{ proposal_id, campaign_id, ad_group_id, proposed_keyword, proposed_match_type }] }` to **`NEXT_PUBLIC_KW_FEEDBACK_WEBHOOK_URL`** (an n8n webhook). The receiving scenario matches each `approved[]` item (by `proposal_id`) and writes `Approved` to that row's `approval` cell. Without the env var the submit errors out. Local hook state is optimistically updated (approved keywords flip to `Approved`).
...
- `NEXT_PUBLIC_KW_FEEDBACK_WEBHOOK_URL` - n8n webhook URL for the Keyword Buildout review panel (the webhook node must use HTTP method POST). Receives `{ batch_id, account_id, account_name, logged_at, approval_status: 'Approved', approved: [{ proposal_id, campaign_id, ad_group_id, proposed_keyword, proposed_match_type }] }`. The n8n workflow matches each `approved[]` item (by `proposal_id`) against the `KW Buildout Proposals` sheet and writes `Approved` to that row's `approval` cell. Without this var the "Approve selected" button errors out. Must be `NEXT_PUBLIC_`.
